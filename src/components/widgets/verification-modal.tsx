...
// Helper to create a slightly darker shade for hover states
function getDarkerShade(hex: string): string {
  // Remove # if present
  const color = hex.replace("#", "");
  const r = Math.max(0, parseInt(color.substring(0, 2), 16) - 20);
  const g = Math.max(0, parseInt(color.substring(2, 4), 16) - 20);
  const b = Math.max(0, parseInt(color.substring(4, 6), 16) - 20);
  ...
  return (
    <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
      ...
      <div
        ...
        style={{ background: `linear-gradient(to bottom right, ${brandColor}, ${darkerShade})` }}
      >
      ...
      <Link
        ...
        target="_blank"
        className="flex items-center gap-1 text-xs font-medium flex-shrink-0 hover:opacity-80 transition-opacity"
            ...
            <div className="p-4">
              <div className="mb-3">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Below are {Math.min(participants.length, 3)} of {study.participantCount}{" "}
