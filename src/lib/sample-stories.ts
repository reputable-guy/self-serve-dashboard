...
import { TierLevel, AssessmentResult } from "./assessments";
...
// Helper to create assessment result
function createAssessmentResult(
  ...
  const compositeChange = endpointComposite - baselineComposite;
  const compositePercent = Math.round((compositeChange / baselineComposite) * 100);
  const rawPercent = Math.round(((endpointRaw - baselineRaw) / baselineRaw) * 100);
  ...
  return {
    ...
    baseline: {
      ...
      primaryScore: Math.round((baselineRaw / maxRaw) * 100),
      primaryRaw: baselineRaw,
      primaryMax: maxRaw,
      responses: [],
    },
    endpoint: {
      ...
      primaryScore: Math.round((endpointRaw / maxRaw) * 100),
      primaryRaw: endpointRaw,
      primaryMax: maxRaw,
      responses: [],
    },
    change: {
      ...
      compositePercent,
      primaryPoints: Math.round((endpointRaw / maxRaw) * 100) - Math.round((baselineRaw / maxRaw) * 100),
      primaryPercent: rawPercent,
    },
    improved: compositeChange > 0,
    headline: `${categoryLabel} improved from ${baselineRaw}/${maxRaw} to ${endpointRaw}/${maxRaw} (+${rawPercent}%)`,
  };
}
...
export const SAMPLE_STORIES_BY_CATEGORY: Record<string, CategorySampleStory> = {
  ...
  energy: {
    ...
    story: {
      ...
      wearableMetrics: {
        device: "Apple Watch",
        sleepChange: { before: 360, after: 390, unit: "min", changePercent: 8 },
  ...
  gut: {
    ...
    story: {
      ...
      wearableMetrics: {
        device: "Apple Watch",
        sleepChange: { before: 370, after: 395, unit: "min", changePercent: 7 },
      },
      assessmentResult: createAssessmentResult(
        "reputable-gut",
  ...
  skin: {
    ...
    story: {
      ...
      wearableMetrics: {
        device: "Oura Ring",
        sleepChange: { before: 385, after: 405, unit: "min", changePercent: 5 },
      },
      assessmentResult: createAssessmentResult(
        "reputable-skin",
  ...
  weight: {
    ...
    story: {
      ...
      wearableMetrics: {
        device: "Garmin",
        sleepChange: { before: 355, after: 380, unit: "min", changePercent: 7 },
      },
      assessmentResult: createAssessmentResult(
        "reputable-weight",
        ...
        38, // baseline composite
        72, // endpoint composite
        7,  // baseline raw (cravings intensity 7/10 - high)
        3,  // endpoint raw (cravings intensity 3/10 - low)
        10
      ),
      verified: true,
  ...
  immunity: {
    ...
    story: {
      ...
      wearableMetrics: {
        device: "Fitbit",
        sleepChange: { before: 365, after: 390, unit: "min", changePercent: 7 },
      },
      assessmentResult: createAssessmentResult(
        "reputable-immunity",
  ...
  hair: {
    ...
    story: {
      ...
      wearableMetrics: {
        device: "Apple Watch",
        sleepChange: { before: 340, after: 375, unit: "min", changePercent: 10 },
      },
      assessmentResult: createAssessmentResult(
        "reputable-hair",
